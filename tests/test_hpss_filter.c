...
void test_hpss_filter(void) {
  ...
  for (uint32_t frame = 0; frame < 20; ++frame) {
    bool ok = hpss_filter_process(filter, current_mag, mask_h, mask_p);
  ...
  TEST_ASSERT(hpss_filter_process(NULL, current_mag, mask_h, mask_p) == false,
              "NULL filter process fails");
  filter = hpss_filter_initialize(config);
  TEST_ASSERT(hpss_filter_process(filter, NULL, mask_h, mask_p) == false,
